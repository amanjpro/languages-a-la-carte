...
trait TreeFactories extends sana.primj.ast.TreeFactories {
  ...
  def mkArrayDef(name: Name, indices: List[TypeUseApi], 
  ...
  def mkForEach(inits: List[Tree], allOrEntries: primj.ast.ApplyApi,
   cond: Expr, body: Expr, symbol: Option[Symbol]): ForEach = {
